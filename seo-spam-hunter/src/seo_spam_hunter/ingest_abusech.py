...
def fetch_urlhaus(
    ...
    try:
        with httpx.Client(timeout=timeout, headers={"User-Agent": "seo-spam-hunter/0.1.0"}) as client:
    ...
    try:
        with httpx.Client(timeout=timeout, headers={"User-Agent": "seo-spam-hunter/0.1.0"}) as client:
            resp = client.post(
                ...
                content=payload,
                headers={"Content-Type": "application/json", "User-Agent": "seo-spam-hunter/0.1.0"},
            )
            resp.raise_for_status()
    except httpx.HTTPError as exc:
