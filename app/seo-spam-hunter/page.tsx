...
export default function SeoSpamHunterPage() {
  ...
  const fetchCampaigns = async () => {
    try {
      const res = await fetch("/api/seo-spam-hunter/campaigns")
  ...
  const fetchFindings = async (cid?: string) => {
    try {
      ...
      const url = targetCid
        ? `/api/seo-spam-hunter/findings?campaignId=${encodeURIComponent(targetCid)}`
        : "/api/seo-spam-hunter/findings"
  ...
  const fetchReports = async (dir?: string) => {
    try {
      const url = dir
        ? `/api/seo-spam-hunter/reports?dir=${encodeURIComponent(dir)}`
        : "/api/seo-spam-hunter/reports"
      const res = await fetch(url)
      const data = await res.json()
      if (data.reports) {
  ...
  const handleSaveCampaign = async (campaignId: string, updatedFields: Partial<PipelineCampaign>) => {
    try {
      const res = await fetch(`/api/seo-spam-hunter/campaigns/${encodeURIComponent(campaignId)}`, {
        method: "PATCH",
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-base font-bold tracking-tight">WordPress SEO Spam & Backdoor Hunter</h1>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Load PublicWWW exports, toggle VT graph pivoting, and launch live threat intelligence feeds.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Paste lines (e.g. infected-site.com, 1500, match snippet) or upload CSV export..."
                  className="w-full h-32 rounded-lg border border-input bg-background p-3 text-xs font-mono text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                />
              </div>
            ...
            <div className="space-y-4 flex flex-col justify-between">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  disabled={executing || !selectedCampaignId}
                  className="col-span-2 flex items-center justify-center gap-1.5 rounded-lg border border-border bg-background py-2 text-xs font-medium text-foreground transition-colors hover:bg-accent disabled:opacity-50"
                >
                  <FileText className="size-3.5" />
                  <span>Stage C: Generate Report Only</span>
                </button>
              </div>
            </div>
          </div>
