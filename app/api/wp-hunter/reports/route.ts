...
export async function GET(request: Request) {
  try {
    ...
    const outputBaseDir = path.join(process.cwd(), 'wp-hunter', 'output')
    ...
    if (download === 'csv') {
      return new Response(csvContent, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="wp-hunter-report-${selectedDir}.csv"`,
        },
      })
    }
    ...
    console.error('Failed to read wp-hunter reports:', error)
